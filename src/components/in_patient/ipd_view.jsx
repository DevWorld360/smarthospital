...
import { ipdHistory } from "../../data/data";
...
const IpdView = () => {
  ...
  const filteredData = ipdHistory.filter(
    (item) =>
          ...
          <tbody className="font-Assistant text-base align-top">
            {opdToDisplay.length > 0 ? (
              opdToDisplay.map((request) => (
                <tr key={request.id}>
                  <td className="px-2 py-2 border-b">{request.opdNumber}</td>
                  <td className="px-2 py-2 border-b">{request.caseId}</td>
                  <td className="px-2 py-2 border-b">{request.patientName}</td>
                  ...
                  <td className="px-2 py-2 border-b">
                    {request.symptoms ?? request.symptoms}
                  </td>
